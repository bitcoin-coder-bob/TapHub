#!/bin/bash

# Full Flow Script for Polar Lightning Network setup (lncli + tapcli)

set -e  # Exit on error

# Source the configuration
source "$(dirname "$0")/config.sh"

echo "Step 0: Verify lnd and tapd Status"
lncli_alice getinfo | jq -r '.alias'
tapcli_alice assets list | jq -r '.assets | length'

echo -e "\nStep 1: Generate a New On-Chain Address in lnd"
lncli_alice newaddress p2wkh

echo -e "\nStep 2: List existing assets (tapcli)"
tapcli_alice assets list | jq -r '.assets[] | "\(.asset_genesis.name): \(.asset_genesis.asset_id)"'

echo -e "\nStep 3: Get asset details"
ASSET_INFO=$(tapcli_alice assets list | jq -r '.assets[0]')
ASSET_ID=$(echo "$ASSET_INFO" | jq -r '.asset_genesis.asset_id')
ASSET_NAME=$(echo "$ASSET_INFO" | jq -r '.asset_genesis.name')
echo "Using asset: $ASSET_NAME"
echo "Asset ID: $ASSET_ID"

echo -e "\nStep 4: List current peers"
lncli_alice listpeers

echo -e "\nStep 5: List current channels"
lncli_alice listchannels

echo -e "\nStep 6: Verify Asset Balance"
tapcli_alice assets balance

echo -e "\nStep 7: Generate Asset-Denominated Address"
tapcli_alice addrs new --asset_id "$ASSET_ID" --amt 50

echo -e "\nStep 8: Create a regular Lightning invoice (sats)"
lncli_alice addinvoice --memo "Test Lightning payment" --amt 1000

echo -e "\nStep 9: List Lightning payments"
lncli_alice listpayments

echo -e "\nStep 10: Verify Asset Balance again"
tapcli_alice assets balance

echo -e "\nFlow demonstration complete!"
echo "This demonstrates both Lightning (sats) and Taproot Assets capabilities."
